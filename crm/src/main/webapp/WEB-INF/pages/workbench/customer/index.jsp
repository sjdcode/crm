...
<script type="text/javascript">
	...
	$(function(){
		...
		//页面首次加载
		queryCustomerForPageByConditon(1,5);

		//点击按钮query
		$("#queryBtn").click(function (){
			queryCustomerForPageByConditon(1,$("#pagination").bs_pagination("getOption","rowsPerPage"));
		})
		...
		//创建model的nextContactTime输入框使用日历插件
		...
		$("#editCustomerBtn").click(function (){
			//判断选择个数
			var chooseNum=$("#tBody input[type='checkbox']:checked").size();
			if(chooseNum>1){
				alert("一次只能修改一个客户信息");
				return;
			}else if(chooseNum===0){
			...
			//获取选择的CustomerId
			var customerId=$("#tBody input[type='checkbox']:checked")[0].value;
			//数据回显
			$.ajax({
				url:'workbench/customer/queryCustomerForDetailById.do',
				data:{
					id:customerId
				},
				type:'post',
		...
		$("#saveEditCustomerBtn").click(function (){
			...
			var nextContactTime=$.trim($("#create-nextContactTime2").val());
			...
			$.ajax({
				...
				success:function (data){
					if(data.code==='0'){
						...
						queryCustomerForPageByConditon($("#pagination").bs_pagination("getOption","currentPage"),
		...
		$("#deleteCustomerBtn").click(function (){
			//收集参数
			var id="";
			var customerList=$("#tBody input[type='checkbox']:checked");
			for (let i = 0; i < customerList.size(); i++) {
				id+="id="+customerList[i].value+"&";
			}
	...
	//查询主界面CustomerList函数
	function queryCustomerForPageByConditon(pageNo,pageSize){
		//收集参数
		var name=$("#query_name").val();
		var owner=$("#query_owner").val();
		var phone=$("#query_phone").val();
		var website=$("#query_website").val();

		//发送请求
		$.ajax({
			url:'workbench/customer/queryCustomerForPageByConditon.do',
			data:{
				name:name,
				owner:owner,
				phone:phone,
			...
			success:function (data){
				...
				$.each(data.customerList,function (index,obj){
					customerListStr+="<tr>";
					customerListStr+="<td><input type=\"checkbox\" value=\""+obj.id+"\" /></td>";
					customerListStr+="<td><a style=\"text-decoration: none; cursor: pointer;\" onclick=\"window.location.href='workbench/customer/toDetail.do?id="+obj.id+"';\">"+obj.name+"</a></td>";
					customerListStr+="<td>"+obj.owner+"</td>";
					if(obj.phone===null){
						customerListStr+="<td></td>";
					}else{
						customerListStr+="<td>"+obj.phone+"</td>";
					}
					if(obj.website===null){
						customerListStr+="<td></td>";
					}else{
						customerListStr+="<td>"+obj.website+"</td>";
					}
					customerListStr+="</tr>";
				})
				$("#tBody").html(customerListStr);
				...
				$("#pagination").bs_pagination({
					...
					onChangePage:function (event,pageObj){
						//切换页面后,取消全选
						$("#chooseAll").prop("checked",false);
						queryCustomerForPageByConditon(pageObj.currentPage,pageObj.rowsPerPage);
					}
				})
			}
		})
					...
					<form class="form-horizontal" id="createCustomerForm" role="form">
                        ...
                        <div style="position: relative;top: 15px;">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-10" style="width: 300px;">
                                    <input type="text" class="form-control" readonly="readonly" id="create-nextContactTime">
                                </div>
                            </div>
                        </div>
				...
				<form class="form-inline" role="form" style="position: relative;top: 8%; left: 5px;">
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <input class="form-control" id="query_phone" type="text">
				    </div>
				  </div>
				  ...
				  <div class="form-group">
				    <div class="input-group">
				      ...
				      <input class="form-control" id="query_website" type="text">
				    </div>
				  </div>
